...
const PostComment = ({ comment, manyPopUps, deleteCommentEvent }) => {
    ...
    return (<div key={comment.id} className="comment">
        ...
        <div className="inner-comment">
            <div className="top-inner-comment">
                <Link to={'/profile/' + comment.commentorUsername}>
                    <div className="top-comment-half">
                        {comment.commentorName}
                    </div>
                </Link>
                {loggedIn.userName === comment.commentorUsername ? <button onClick={() => deleteCommentEvent(comment.id)}>Delete</button> : ''}
            </div>
